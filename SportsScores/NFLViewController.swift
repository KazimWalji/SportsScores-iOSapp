...
class NFLViewController: UIViewController, UICollectionViewDataSource, UITableViewDataSource, UITableViewDelegate, UICollectionViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let calendarConstraints = [calendarView.topAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.topAnchor, constant: 10), calendarView.leadingAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.leadingAnchor, constant: 15), calendarView.trailingAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.trailingAnchor, constant: -15), calendarView.heightAnchor.constraint(equalToConstant: 50)]
        NSLayoutConstraint.activate(calendarConstraints)
        
        do {
            if let calendar = try APIData.getNFLCalendar() {
        ...
        let tableViewConstraints = [tableView.topAnchor.constraint(equalTo: calendarView.bottomAnchor, constant: 0), tableView.bottomAnchor.constraint(equalTo: self.view.bottomAnchor, constant: -100), tableView.centerXAnchor.constraint(equalTo: self.view.centerXAnchor, constant: 0), tableView.widthAnchor.constraint(equalTo: self.view.widthAnchor, constant: 0)]
        NSLayoutConstraint.activate(tableViewConstraints)
        do {
            if let allGames = try APIData.getData(date: Date(), nba: false) {
                games = allGames
            }
        }
        catch {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if !loaded {
            ...
            if !found {
                if let closestDate = calendar.sorted().first(where: {$0.timeIntervalSinceNow > 0}) {
                ...
                do {
                    ...
                    if let allGames = try APIData.getData(date: closestDate, nba: false) {
                        games = allGames
                        self.tableView.reloadData()
                    }
                }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if selectedIndex != indexPath.row {
            do {
                if let allGames = try APIData.getData(date: createDate.from(year: calendar[indexPath.row].get(Calendar.Component.year), month: calendar[indexPath.row].get(Calendar.Component.month), day: calendar[indexPath.row].get(Calendar.Component.day)), nba: false) {
                    games = allGames
                    self.tableView.reloadData()
                }
            }
